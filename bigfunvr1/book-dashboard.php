...
if ($categories_result->num_rows > 0) {
    while ($row = $categories_result->fetch_assoc()) {
        // Split categories by comma and trim whitespace from each
        $cats = array_map('trim', explode(',', $row['service_category']));
        // Add the individual categories to our master list
        $all_categories = array_merge($all_categories, $cats);
    }
}
// Filter out any empty values, get unique categories, and sort them
...
// ❗ DEACTIVATED: This logic is now handled by backend/add_to_cart.php via JavaScript Fetch
/*
if (isset($_POST['add_to_cart'])) {
    // ... old session logic was here ...
}
*/
...
<head>
  ...
  <style>
    :root { --primary-color: #802080; --primary-light: #FFEFFC; --primary-dark: #661966; }
    body { font-family: 'Inter', sans-serif; margin: 0; padding: 0; background: var(--primary-light); }
    ...
    .btn-cart { padding: 10px 20px; background: var(--primary-color); color: #fff; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-family: 'Inter', sans-serif; align-items: center; justify-content: center; }
    ...
    .category-sidebar { background: #fff; border-radius: 10px; border: 1px solid #ddd; }
    .category-sidebar a { text-decoration: none; color: #333; font-size: 15px; transition: color 0.2s; }
    .category-sidebar a:hover { color: var(--primary-color); font-weight: 600; }
    .category-sidebar a.active { color: var(--primary-color) !important; font-weight: 700; }

    .product-card-clickable { border: 1px solid #ddd; border-radius: 12px; background-color: #8C367C; padding: 15px; text-align: center; position: relative; color: #fff; font-family: inter, sans-serif; cursor: pointer; transition: transform 0.2s ease-in-out; }
    .product-card-clickable:hover { transform: translateY(-5px); }
    .product-card-clickable img { width: 100%; height: 200px; object-fit: contain; margin-bottom: 10px; }

    #serviceDetailModal .modal-price { color: var(--primary-color); font-size: 2rem; font-weight: 700; }
    #serviceDetailModal .modal-category-badge { background-color: var(--primary-color); }
    #serviceDetailModal .accordion-button:not(.collapsed) { background-color: #f8f0f8; color: var(--primary-color); box-shadow: none; }
    #serviceDetailModal .btn-book-modal { background-color: var(--primary-color); color: white; font-weight: 600; padding: 0.75rem 1.5rem; }
...
<div class="container-fluid" style="padding:20px 6%;">
  <div class="row">
    <aside class="col-md-3 d-none d-md-block">
       <div class="category-sidebar d-flex flex-column text-center p-4" style="background:#fff; border-radius:8px; min-height:100%; box-shadow:0 2px 8px rgba(0,0,0,0.05);">
        <h6 class="mb-4 text-uppercase fw-bold" style="color:#802080;">Services Category</h6>
        <?php foreach ($categories as $category): ?>
        <a href="book-dashboard.php?category=<?php echo urlencode($category); ?>" class="d-block mb-3 fw-medium text-dark text-decoration-none <?php echo ($filter === $category) ? 'active' : ''; ?>"><?php echo htmlspecialchars($category); ?></a>
        <?php endforeach; ?>
    ...
    <main class="col-md-9">
        <h3 class="mb-4"><?php echo htmlspecialchars($filter); ?></h3>
        <div class="row g-4">
            ...
            <?php else: ?>
              <?php foreach ($filtered_products as $id => $p): ?>
                <div class="col-md-4">
                  <div class="product-card-clickable h-100"
                    ...
                    <img src="uploads/<?php echo htmlspecialchars($p['service_image']); ?>" alt="<?php echo htmlspecialchars($p['service_name']); ?>">
                    <h4><?php echo htmlspecialchars($p['service_name']); ?></h4>
...
<div class="modal fade" id="serviceDetailModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header border-0"><h5 class="modal-title">Service Details</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <div class="accordion accordion-flush" id="detailsAccordion">
                        <div class="accordion-item"><h2 class="accordion-header"><button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseDesc">Description</button></h2><div id="collapseDesc" class="accordion-collapse collapse show" data-bs-parent="#detailsAccordion"><div class="accordion-body" id="modalDescription"></div></div></div>
                        <div class="accordion-item"><h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSpec">Specification</button></h2><div id="collapseSpec" class="accordion-collapse collapse" data-bs-parent="#detailsAccordion"><div class="accordion-body" id="modalSpecification"></div></div></div>
                        <div class="accordion-item"><h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseAdd">Additional</button></h2><div id="collapseAdd" class="accordion-collapse collapse" data-bs-parent="#detailsAccordion"><div class="accordion-body" id="modalAdditional"></div></div></div>
                    </div>
                    <div class="mt-4">
                        ...
                        <div class="input-group" style="width: 150px;">
                            ...
                            <input type="text" class="form-control text-center" value="1" name="quantity" readonly>
...
<div class="toast-container position-fixed top-0 end-0 p-3">
    <div id="cartToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true">
        ...
        <div class="toast-body">Service added to cart successfully!</div>
    </div>
</div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
      ...
      serviceDetailModalEl.addEventListener('show.bs.modal', function(event) {
          ...
          const service = { id: card.dataset.id, name: card.dataset.name, price: card.dataset.price, image: card.dataset.image, category: card.dataset.category, description: card.dataset.description, specification: card.dataset.specification, additional: card.dataset.additional };
          ...
          serviceDetailModalEl.querySelector('#modalCategory').textContent = service.category;
          ...
          serviceDetailModalEl.querySelector('input[name="quantity"]').value = 1;
      });

      // --- ❗ NEW: Handle Add to Cart with JavaScript Fetch ---
      addToCartForm.addEventListener('submit', function(event) {
          event.preventDefault(); // Stop the form from reloading the page
          const formData = new FormData(addToCartForm);
          fetch('backend/add_to_cart.php', { method: 'POST', body: formData })
      ...
      serviceDetailModalEl.addEventListener('click', function(e){
          ...
          if(e.target.id === 'modal-quantity-plus') quantityInput.value = currentValue + 1;
          if(e.target.id === 'modal-quantity-minus' && currentValue > 1) quantityInput.value = currentValue - 1;
      });
  });
</script>
</body>
