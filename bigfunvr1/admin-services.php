...
<head>
    ...
    <style>
        ...
        .highlight { background-color: #fce4fc !important; transition: background-color 0.5s; }
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="action-bar d-flex justify-content-between align-items-center">
            ...
            <button class="btn btn-outline-secondary d-flex align-items-center" type="button" data-bs-toggle="offcanvas" data-bs-target="#quickFindOffcanvas">
                <span class="material-symbols-outlined me-1">list</span> Quick Find
            </button>
    ...
    <div class="offcanvas offcanvas-start" tabindex="-1" id="quickFindOffcanvas"><div class="offcanvas-header"><h5 class="offcanvas-title">Quick Find</h5><button type="button" class="btn-close" data-bs-dismiss="offcanvas"></button></div><div class="offcanvas-body"><p>Click a service to scroll to it in the table.</p><div class="list-group list-group-flush" id="quickFindList"></div></div></div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const quickFindList = document.getElementById('quickFindList');
            const quickFindOffcanvasEl = document.getElementById('quickFindOffcanvas');
            const quickFindOffcanvas = new bootstrap.Offcanvas(quickFindOffcanvasEl);

            // --- Populate Quick Find List ---
            function populateQuickFind() {
                quickFindList.innerHTML = '';
                servicesTableBody.querySelectorAll('tr').forEach(row => {
                    if (row.style.display !== 'none' && row.id !== 'no-services-row') {
                        const serviceName = row.querySelector('td:nth-child(2)').textContent;
                        const serviceId = row.id;
                        const listItem = document.createElement('a');
                        listItem.className = 'list-group-item list-group-item-action';
                        listItem.textContent = serviceName;
                        listItem.href = `#${serviceId}`;
                        listItem.dataset.targetId = serviceId;
                        quickFindList.appendChild(listItem);
                    }
                });
            }
            ...
            // --- Quick Find Click Functionality ---
            quickFindList.addEventListener('click', function(e) {
                if (e.target.matches('.list-group-item')) {
                    e.preventDefault();
                    const targetRow = document.getElementById(e.target.dataset.targetId);
                    if (targetRow) {
                        quickFindOffcanvas.hide();
