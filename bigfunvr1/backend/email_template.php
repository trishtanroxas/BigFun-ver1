...
function generateOrderEmailBody($order_id, $user, $cart_items, $order_details) {
    ...
    // --- 1. Prepare Order Items & Financial Summary ---
    $subtotal = 0;
    $items_html = '';
    ...
    $billing_summary_html = "<tr class='total-row'><td>Subtotal</td><td>$" . number_format($subtotal, 2) . "</td></tr>";
    if ($order_details['payment_method'] === 'Installment') {
        $interest_amount = $order_details['total_amount'] - $subtotal;
        $billing_summary_html .= "<tr class='total-row'><td>Interest</td><td>$" . number_format($interest_amount, 2) . "</td></tr>";
    }
    $billing_summary_html .= "<tr class='total-row grand-total'><td>Grand Total</td><td>$" . number_format($order_details['total_amount'], 2) . "</td></tr>";

    // --- 2. Prepare Detailed Information Sections ---
    $masked_card_info = '';
    ...
    $personal_info_html = format_detail_row('Full Name', $order_details['full_name'])
                        . format_detail_row('Address', $order_details['address'])
    ...
    $event_details_html = format_detail_row('Event Date', date('F j, Y', strtotime($order_details['date_event'])))
                        ...
                        . format_detail_row('Location Notes', $order_details['location_checklist'])
    ...
    $payment_details_html = format_detail_row('Payment Method', $order_details['payment_method'])
                          ...
                          . format_detail_row('Interest Rate', !empty($order_details['interest_rate']) ? $order_details['interest_rate'] . '%' : '')
                          . format_detail_row('Next Payment Due', !empty($order_details['next_due_date']) ? date('F j, Y', strtotime($order_details['next_due_date'])) : '');

    // --- 3. Prepare Dynamic Call-to-Action ---
    $call_to_action_html = '';
    ...
    // --- 4. Define CSS Styles for the Email ---
    $style = "
        ...
        .total-summary .grand-total td { font-weight: bold; font-size: 1.3em; color: #802080; border-top: 2px solid #333; padding-top: 10px; }
    ...
    // --- 5. Assemble the Final HTML Email ---
    return <<<HTML
    <!DOCTYPE html>
    <html>
    <head>
