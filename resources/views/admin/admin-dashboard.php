...
<head>
    ...
    <style>
        ...
        .card-title-container .material-symbols-outlined { font-size: 2rem; }
        ...
        .card-footer-link { margin-top: auto; padding-top: 0.75rem; font-size: 0.85rem; font-weight: 600; text-decoration: none; position: relative; z-index: 2; }
        .card-footer-link:hover { color: var(--primary-color); }
        .card-sub-stats { display: flex; justify-content: space-between; align-items: center; margin-top: auto; padding-top: 0.75rem; font-size: 0.9rem; }
        .card-sub-stats span { display: flex; align-items: center; gap: 0.25rem; }
        .card-sub-stats .material-symbols-outlined { font-size: 1rem; }
        ...
        <div class="row">
            <div class="col-xl-3 col-md-6">
                <div class="dashboard-card">
                    <div class="card-title-container"><span class="material-symbols-outlined" style="color: var(--primary-color);">request_quote</span><h5>Total Revenue</h5></div>
                    <p class="card-value" id="revenueSummary">AU$ 0</p>
                    <a href="#" class="card-footer-link" data-details-type="revenue_details">See Details</a>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="dashboard-card">
                    <div class="card-title-container"><span class="material-symbols-outlined" style="color: var(--info);">summarize</span><h5>Appointments in Period</h5></div>
                    <p class="card-value" id="totalAppointments">0</p>
                    <div class="card-sub-stats text-muted">
                        <span><span class="material-symbols-outlined" style="color: var(--success);">event_available</span>Upcoming: <strong id="upcomingInPeriod" class="text-dark">0</strong></span>
                        <span><span class="material-symbols-outlined" style="color: var(--dark-gray);">event_busy</span>Cancelled: <strong id="cancelledInPeriod" class="text-dark">0</strong></span>
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="dashboard-card">
                    <div class="card-title-container"><span class="material-symbols-outlined" style="color: var(--warning);">hourglass_top</span><h5>Partially Paid</h5></div>
                    <p class="card-value" id="partiallyPaidOrders">0</p>
                    <a href="#" class="card-footer-link" data-details-type="invoices_partial">See Details</a>
                </div>
            </div>
            <div class="col-xl-3 col-md-6">
                <div class="dashboard-card">
                    <div class="card-title-container"><span class="material-symbols-outlined" style="color: var(--danger);">error</span><h5>Unpaid Invoices</h5></div>
                    <p class="card-value" id="unpaidOrders">0</p>
                    <a href="#" class="card-footer-link" data-details-type="invoices_unpaid">See Details</a>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    let charts = {};
    let currentFilter = { type: 'period', value: 'month' };
    const detailsModal = new bootstrap.Modal(document.getElementById('detailsModal'));

    const STATUS_COLORS = { 'Paid': '#198754', 'Partially Paid': '#ffc107', 'Unpaid': '#dc3545' };
    const CHART_COLORS = { purple: '#802080', pink: 'rgba(128, 32, 128, 0.1)', yellow: '#ffc107', yellow_light: 'rgba(255, 193, 7, 0.1)', blue: '#0dcaf0' };
    ...
    const fetchAnalytics = async (filter) => {
        ...
        if (filter.type === 'period') { url += `period=${filter.value}`; } 
        else if (filter.type === 'range') { url += `startDate=${filter.start}&endDate=${filter.end}`; }

        try {
            const response = await fetch(url);
            if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
            const data = await response.json();
            if (data.status === 'success') { updateUI(data.data); } 
    ...
    const updateUI = (data) => {
        const manageChartState = (chartId, chartData, createChartCallback) => {
            ...
            if (!wrapper) return;
            let hasData = (chartData && chartData.labels && chartData.labels.length > 0 && chartData.values.some(v => v > 0));
            if (hasData) {
                ...
                wrapper.innerHTML = `<div class="d-flex align-items-center justify-content-center h-100 text-muted fst-italic p-3">No payment data for this period.</div>`;
            }
        };

        // ✨ MODIFIED: Update all KPI Card values
        ...
        // Update new combined appointments card
        document.getElementById('totalAppointments').textContent = data.kpis.total_appointments;
        document.getElementById('upcomingInPeriod').textContent = data.kpis.upcoming_in_period;
        document.getElementById('cancelledInPeriod').textContent = data.kpis.cancelled_in_period;
        ...
        createOrUpdateChart('revenueChart', 'line', {
            ...
            datasets: [
                { label: 'Total Revenue', data: data.revenue_chart.datasets.paid, borderColor: CHART_COLORS.purple, backgroundColor: CHART_COLORS.pink, fill: true, tension: 0.3 },
                { label: 'Partial Payments', data: data.revenue_chart.datasets.partial, borderColor: CHART_COLORS.yellow, backgroundColor: CHART_COLORS.yellow_light, fill: false, tension: 0.3 }
            ]
        ...
        manageChartState('statusChart', data.status_chart, () => {
            createOrUpdateChart('statusChart', 'doughnut', {
                ...
                datasets: [{ 
                    ...
                    backgroundColor: data.status_chart.labels.map(label => STATUS_COLORS[label] || '#cccccc'),
                    borderColor: '#fff', 
                    borderWidth: 4 
                }]
        ...
        // Use a separate manage state for services to show a different message if no services booked
        const manageServiceChart = (chartId, chartData, createChartCallback) => {
             const wrapper = document.getElementById(chartId)?.parentElement;
            if (!wrapper) return;
            if (chartData && chartData.values && chartData.values.length > 0) {
                ...
                wrapper.innerHTML = `<div class="d-flex align-items-center justify-content-center h-100 text-muted fst-italic p-3">No services booked in this period.</div>`;
            }
        };

        manageServiceChart('servicesChart', data.services_chart, () => {
    ...
    document.querySelectorAll('a[data-details-type]').forEach(link => {
        link.addEventListener('click', async (e) => {
            ...
            if (currentFilter.type === 'period') { url += `&period=${currentFilter.value}`; } 
            else if (currentFilter.type === 'range') { url += `&startDate=${currentFilter.start}&endDate=${currentFilter.end}`; }
                    ...
                    if (data.details.length > 0) {
                        ...
                        data.details.forEach(row => {
                            ...
                            Object.entries(row).forEach(([key, val]) => {
                                ...
                                if (key.includes('amount') || key.includes('balance') || key.includes('revenue')) {
